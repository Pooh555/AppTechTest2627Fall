...
## I-1 (2026-09-24 pass) — Advanced filter icon not centered in search bar

- **Reproduction:** in `BrowseScreen`, the "Filters" control was a text `Pressable` in a
  separate `View` (`$filters`) below the `TextInput`, not inline with it, so there was no
  shared cross-axis to align against; it also rendered as text, not the "3-line" icon the
  ticket describes.
- **Root cause (confirmed, `app/screens/BrowseScreen.tsx`):** the search input and the filter
  control lived in sibling rows instead of one `flexDirection: 'row'` container, so no
  `alignItems: 'center'` could apply between them, and the filter control had no fixed
  height, letting its own vertical padding drift from the `TextInput`'s.
- **Decision:** wrap the `TextInput` and a new icon button (`Icon` registry's existing
  `menu` — a 3-line glyph) in one `$searchRow` (`flexDirection: 'row', alignItems: 'center'`),
  give the `TextInput` `flex: 1` with `margin: 0`, and give the icon button a fixed
  `44x44` box with `alignItems/justifyContent: 'center'` and `margin: 0`, so both children
  share the row's cross-axis center with no conflicting margins.
- **Evidence:** `PressableIcon` box is a constant 44x44, independent of font scale, and both
  it and `TextInput` sit in the same row with `alignItems: 'center'`, satisfying the
  centering requirement across viewports/screen densities without per-platform branching.

## I-2 (2026-09-24 pass) — Term filter chips did not restrict results

- **Reproduction:** could not exercise the compiled RN app or Maestro in this sandbox (no
  network access, no `node_modules`, so `expo`/`better-sqlite3`/Maestro are unavailable).
  Instead, reproduced the data path directly: rebuilt the real `courses` +
  `course_terms` tables in-memory (Node's built-in `node:sqlite`) from the actual
  `courses.json` fixture using the exact `dedupeCourses`/`insertCourses` logic in
  `scripts/pipeline.ts` / `scripts/build-data.ts`, then ran the term-filter SQL that
  `buildSearchPlan` generated. **Fact:** that query already narrowed 4,030 courses to 3,928
  for one term code and 3,639 for another — i.e. the SQL against `course_terms` was not
  silently a no-op.
- **Root cause (confirmed by reading, `app/lib/search/searchIntent.ts` line ~86, prior to
  this fix):** the term clause filtered against `course_terms`, which is populated purely
  from the catalog snapshot rows in `courses.json` (i.e. "this course was catalogued for
  term X"), not from `sections` (i.e. "this course actually has a scheduled section in
  term X"). The ticket's own DESIGN step calls for filtering via
  `courses.code IN (SELECT DISTINCT course_code FROM sections WHERE term IN (...))` and an
  index on `sections(term, course_code)` — i.e. the intended source of truth for "offered
  in a term" is the schedule table, not the catalog listing. Filtering against the catalog
  listing let courses that are nominally catalogued for a term, but have no actual section
  running, remain visible — matching the reported symptom.
- **Decision:** changed the term-filter clause in `buildSearchPlan` to
  `c.code IN (SELECT DISTINCT s.course_code FROM sections s WHERE s.term_code IN (...))`;
  added `idx_sections_term_code ON sections(term_code, course_code)` in `build-data.ts` (a
  build script, not a generated artifact) so the term-leading lookup stays indexed.
- **Test added:** `app/lib/search/searchPlan.test.ts` now executes the generated SQL
  against the real bundled `courses.db` and asserts (a) the term-filtered result set is
  strictly smaller than the unfiltered set and (b) every returned code actually has a row
  in `sections` for that term — closing the gap where the previous test only asserted on
  the SQL string, never executed it.
- **Residual risk / could not verify:** I could not run `npm test`, `tsc`, `eslint`,
  `depcruise`, or the Maestro flow in this environment (no network, no installed
  dependencies, `assets/data/courses.db` is generated by `build:data` and wasn't present in
  the provided archive). This fix and test should be run through the full pipeline
  (`npm run build:data && npm test && npm run compile && npm run lint:check && npm run
  depcruise && npm run test:maestro`) before merging. If the environment that runs
  `build:data` has not been re-run with a `schedule.json` that actually populates
  `sections`, this change will make term filters return zero rows (regression) rather than
  under-filtering — worth confirming `sections` is populated for all terms before deploy.
